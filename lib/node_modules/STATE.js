...
function STATE (address, modulepath, dependencies) {
  ...
  function validate_and_preprocess ({ fallback, xtype, pre_data = {}, orphan_check, fun_status, entries }) {
    ...
    let fallback_data = fallback(status.args[pre_id], { listify: tree => listify(tree, modulepath), tree: status.tree_pointers[modulepath], address })
    ...
    if(fun_status.overrides[pre_id]){
      fallback_data = fun_status.overrides[pre_id].fun[0](status.args[pre_id], { listify: tree => listify(tree, modulepath), tree: status.tree_pointers[modulepath] }, get_fallbacks({ fallback_data, modulename: local_status.name, modulepath, instance_path: pre_id }))
      console.log('Override used: ', pre_id, fun_status.overrides[pre_id].by)
      override_address = fun_status.overrides[pre_id].by.at(-1)
    ...
    function sanitize_subs ({ local_id, entry, path, local_tree, xtype, mapping }) {
      ...
      function fill_dataset (dataset, dataset_type) {
        ...
        dataset.id = db.get_id(dataset.id)
    ...
    function sanitize_file (file_id, file, entry, entries) {
      ...
      if(file.$ref){
        ...
        file.addresses = [local_address, override_address]
        // file.$ref = local_address.substring(0, local_address.lastIndexOf("/")) + '/' + file.$ref
      }
      file.id = db.get_id(file.id)
  ...
  if (!result) {
    if (raw === undefined){
      ...
      for(address of addresses){
        response = await fetch(address.substring(0, address.lastIndexOf("/")) + '/'  + $ref, { cache: 'no-store' })
        if (response.ok) break
        console.warn(`Failed to fetch from ${address.substring(0, address.lastIndexOf("/")) + '/'  + $ref}, trying next address...`)
      }
      if (!response.ok) 
...
function get_fallbacks ({ fallback_data, modulename, modulepath, instance_path }) {
  ...
  function register ({ type: dataset_type, name: dataset_name, dataset}) {
    Object.entries(dataset).forEach(([node_id, files]) => {
      ...
      Object.entries(files).forEach(([file_id, file]) => {
        ...
        file.id = db.get_id(local_status.name + '.' + type)
      ...
      new_dataset.id = db.get_id(node.name + '.' + dataset_type + '.dataset')
