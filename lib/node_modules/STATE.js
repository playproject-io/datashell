...
const FAIL = ERROR('https://github.com/playproject-io/datashell/tree/main/doc')
...
/******************************************************************************
  STATE
******************************************************************************/
function STATE (filepath, modulepath, dependencies) {
  status.modulepaths[modulepath] = 0
  status.filepaths[modulepath] = filepath
  console.log(`%c[STATE]`, `color: green;`, { modulepath, filepath })
  const name = extract_filename(filepath)
  const local_status = status.local_statuses[modulepath] = { // Variables (module-level)
    name, modulepath, deny: {}, sub_modules: [], sub_instances: {}
  }
  const statedb = defaults => _statedb(local_status, defaults)
  if (!db) (statedb.admin = admin, db = modulepath)
  return statedb
  /******************************************************************************
    STATEDB
  ******************************************************************************/
  function _statedb (local_status, fallback) {
    if (!status.ROOT_ID) throw new Error(FAIL.admin_access())
...
//    if (!Object.values(status.open_branches).reduce((acc, curr) => acc + curr, 0)) status.inits.forEach(init => init())
    init_module()
    var xtype
    const api = create_statedb_interface(modulepath, 'module', local_status)
    ...
    } catch (error) { throw new Error(FAIL.id({ modulepath, error })) }
  }
  // --------------------------------------------------------------------------
...
//        status.open_branches[super_id]--
        ...
        while (!status.tree_pointers[new_super_id]) {
          ...
          if (temp_name === undefined) throw new Error(FAIL.state_init({
...
//    debugger
    ...
    if (status.fallback_check) {
      ...
      console.log(`%c[STATE]`, `color: green;`, 'Main module: ', statedata.id, '\n', state_entries)
      ...
      extra_fallbacks.length && extra_fallbacks.forEach(([key, value]) => {
        local_status.fallback_instance[key] = (args, tools) => {
          console.log(`%c[STATE]`, `color: green;`, 'Extra fallback: ', statedata.api[key] ? statedata.api[key] : old_fallback[key])
    ...
    if (status.root_module) { //Setup local data (module level)
      ...
      // @INFO: currently nothing happens, because statedata.admins is never set
      // -> but maybe it can be set in fallbacks by an app root that uses STATE?
      statedata.admins && admins.push(...statedata.admins)
      console.log('%c[ADMINS]', 'color: red;', { admins })
    }
  ...
  function init_instance (sid, fallback_key) {
    ...
    if (status.fallback_check) {
      ...
      console.log(`%c[STATE]`, `color: green;`, 'Main instance: ', statedata.id, '\n', state_entries)
      db.append(['state'], state_entries)
    }
    ...
    Object.assign(status.a2i, address2ID)
    Object.assign(status.i2a, ID2Address)
    const api = create_statedb_interface(statedata.id, 'instance', local_status)
...
LESEZEICHEN: 'continue here' // fix demo2/example2
    ...
    if (id && (id.split(':')[0] !== modulepath || !id.includes(':'))) {
      throw new Error(FAIL.invalid_sid({ id, modulepath }))
    }
    if (status.used_ids.has(id)) {
      debugger
      throw new Error(FAIL.sid_used(id))
    }
  ...
  function find_super ({ xtype, fallback, fun_status, local_status }) {
    ...
    if (status.modulepaths[modulepath_super] !== undefined) {
      throw new Error(FAIL.undefined_sub({ modulepath, modulepath_super }))
    }
    ...
    if (xtype === 'module') {
      ...
      console.log(`%c[STATE]`, `color: green;`, data)
    ...
    const opts = { fun_status, fallback, xtype, pre_data: data, orphan_check: true, entries }
    return {
  ...
  function preprocess (opts) {
    const { fallback, xtype, pre_data = {}, orphan_check, fun_status, entries } = opts
    const used_keys = new Set()
    let { id: pre_id, hubs: pre_hubs, mapping } = pre_data
    console.log('%c[STATE:PREPROCESS]', 'color: fuchsia;', pre_id)
    let fallback_data
    const listify = tree => tolist(tree, modulepath)
    const tree = status.tree_pointers[modulepath]
    ...
    try {
      validate_fallback_data(fallback_data)
    } catch (error) {
      throw new Error(FAIL.fallback({ pre_id, xtype, stack: error.stack }))
    }
    ...
    if (overrides) {
      ...
      console.log(`%c[STATE]`, `color: green;`, 'Override used: ', pre_id, overrides.by, )
      overrides.by.splice(0, 1)
      overrides.fun.splice(0, 1)
    }
    // console.log('fallback_data: ', fallback)
    fun_status.overrides = register_overrides({
      overrides: fun_status.overrides, tree: fallback_data, path: modulepath, id: pre_id, filepath
    })
    console.log(`%c[STATE]`, `color: green;`, 'overrides: ', Object.keys(fun_status.overrides))
    ...
    **/
    return {
      sanitized_data: sanitize_state({
        local_id: '', entry: fallback_data, path: pre_id, xtype, mapping, entries
      }),
    ...
    function extract_data ({ local_id, entry, path, hub_entry, xtype, xkey }) {
      if (local_id) {
        ...
        if (xtype === 'instance') {
          ...
          const modulepath = temp_path + local_id
          entry.type = modulepath
          path = modulepath + ':' + xkey
          var temp = Number(xkey)+1
          var temp2 = db.read(['state', path])
          while (temp2 || used_keys.has(path)) {
            path = modulepath + ':' + temp
        ...
        if (xtype === 'instance') {
          entry.type = local_status.modulepath
        } else {
          ...
          // entry.drive[file_id] = { $ref: filepath }
    ...
    function sanitize_file (file_id, file, entry, entries) {
      ...
      if (file.$ref) file.filepath = file.filepath || filepath
    ...
    function sanitize_subs ({ local_id, entry, path, local_tree, xtype, mapping }) {
            ...
            Object.entries(path_def).forEach(([path, value]) =>{
              ...
              if (!new_dataset.id) {
                ...
                if(new_drive.includes(new_dataset.id)){
                  console.warn(`%c[STATE]`, `color: green;`, `Both JSON and path definitions used for dataset "${dataset_type}" in the drive of "${entry.id}"`)
                  return
                }
                Object.assign(new_dataset, fill_dataset(new_dataset, dataset_type))
              }
      ...
      function activate_dataset (dataset, dataset_type) {
        ...
        if (status.root_module) fun_status.root_datasets.push(dataset.type)
          ...
          if(!hub_entry.inputs) throw new Error(FAIL.nodrive(hub_entry.id))
          if(!mapping?.[dataset_type]) throw new Error(FAIL.nomapping({
      ...
      function sanitize_mapping (mapping) {
        ...
        console.log(`%c[STATE]`, `color: green;`, 'Sanitizing mapping: ', mapping, entry.id)
        ...
        Object.entries(mapping).forEach(([key, value]) => {
          ...
          else throw new Error(FAIL.nodataset({ value, subid: entry.id }))
        })
        return new_mapping
      }
    }
...
/*****************************************************************************/
function compare (before, after, filepath) {
  ...
  for (const key in after) {
    if (key === "$ref") {
      ...
      after[key] = compare(before ? before[key] : undefined, after[key], filepath)
    }
  }
  return after
}
...
function verify_imports (id, imports, data) {
  ...
  if (!data._) {
    if (imports.length > 1) {
      ...
      const modulepath = status.local_statuses[id].modulepath
      throw new Error(FAIL.nosubs({ imports, modulepath }))
    }
    else return
  }
  ...
  imports.forEach(imp => {
    let check = true
    fallback_imports.forEach(fallimp => { if (imp === fallimp) check = false })
    if (check) throw new Error(FAIL.module_undefined({
      imp, id: status.local_statuses[id].modulepath
    }))
  })
  fallback_imports.forEach(fallimp => {
    let check = true
    imports.forEach(imp => { if (imp === fallimp) check = false })
    if (check) throw new Error(FAIL.unused_module({
      fallimp, id: status.local_statuses[id].modulepath
    }))
  })
}
/*****************************************************************************/
...
/*****************************************************************************/
function register_overrides ({ overrides, filepath, ...args }) {
  const { id: root_id } = args
  recurse(args)
  return overrides
  function recurse ({ tree, path = '', id, xtype = 'instance', local_modulepaths = {} }) {
    if (tree._) Object.entries(tree._).forEach(([type, instances]) => {
      const sub_path = path + '>' + type
      Object.entries(instances).forEach(([id, branch]) => {
        const resultant_path = id === '$' ? sub_path : sub_path + ':' + id
        const is_type = ['object', 'string'].includes(typeof(branch))
        const cond2 = is_type && id !== 'mapping' && branch._ === undefined
        if (typeof(branch) === 'function') {
          const override = mutate
          function mutate (...all) {
            const before = JSON.parse(JSON.stringify(all[2][0]()))
            const after = compare(before, branch(...all), filepath)
            if (after.api) {
              const api = after.api
              after.api = (...all) => {
                const before = JSON.parse(JSON.stringify(all[2][0]()))
                return compare(before, api(...all), filepath)
              }
            }
            return after
          }
          ...
          else overrides[resultant_path] = {fun: [override], by: [root_id]}
        }
        else if (cond2) status.args[resultant_path] = structuredClone(branch)
        else recurse({ tree: branch, path: sub_path, id, xtype, local_modulepaths })
      })
    })
  }
}
...
/******************************************************************************
  [OK] CREATE_STATEDB_INTERFACE
******************************************************************************/
function create_statedb_interface (node_id, xtype, local_status) {
  const drive = make_drive (node_id)
  const api = make_state_api(node_id, { status, xtype, local_status, drive })
  const admin = status.dataset = make_admin_api({ drive })
  if (node_id === status.ROOT_ID) api.admin = admin
  return api // public_api: sdb
}

/******************************************************************************
  [OK] HELPERS: GET_FALLBACKS
******************************************************************************/
function get_fallbacks ({ fallback_data, overrides, modulepath, instance_path }) {
  ...
  function mutated_fallback () {
    return merge_trees(fallback_data, modulepath)
  }
  // --------------------------------------------------------------------------
  function merge_trees (data, path) {
    if (data._) Object.entries(data._).forEach(recurse)
    else data.$ = { _: status.tree_pointers[path]?._ }
    return data
    function recurse ([type, data]) {
      const sub_path = path + '>' + type.split('$')[0].replace('.', '>')
      return merge_trees(data, sub_path)
    }
  }
  // --------------------------------------------------------------------------
}
/******************************************************************************
  [OK] HELPER: MAKE_INPUT_MAP
******************************************************************************/
...
/******************************************************************************
  [OK] HELPER: GET_INPUT
******************************************************************************/
async function get_input ({ id, name, $ref, type, raw, filepath }) { // @INFO: fetch
  // dependencies: db, window.location, Boolean, window.fetch, FAIL
  ...
  if (!result) {
    if (raw === undefined) {
      ...
      const ref = `${github_project}${filepath.substring(0, filepath.lastIndexOf("/"))}/${$ref}`
      const response = await fetch(ref, { cache: 'no-store' })
      if (!response.ok) throw new Error(FAIL.datafetch({ ref, id }))
      else result = await response[xtype === 'json' ? 'json' : 'text']()
    }
    else result = raw
    db.add(['state', id], { id, name, type, filepath, $ref, raw: result })
  }
  return result
}
/******************************************************************************
  [OK] HELPER: MAKE_ADMIN_API
******************************************************************************/
function make_admin_api ({ drive }) {
  // dependencies: db, window.location.reload, status, console, make_input_map
  ...
  return admin
  // --------------------------------------------------------------------------
  function _populate ([key, value]) { db.add([key], JSON.parse(value), true) }
  // --------------------------------------------------------------------------
  function load (snapshot) {
    db.wash()
    Object.entries(snapshot).forEach(_populate)
    window.location.reload()
  }
  // --------------------------------------------------------------------------
  function register_callback (callbacks) { status.callbacks.push(...callbacks) }
  // --------------------------------------------------------------------------
  function swap ({ type: dataset_type, name: dataset_name = 'default'}) {
...
/******************************************************************************
  [OK] HELP: MAKE_STATE_API
******************************************************************************/
function make_state_api (node_id, { status, xtype, local_status, drive }) {
  const sdb = { id: node_id, watch, drive, get, get_sub }
  return sdb
  // --------------------------------------------------------------------------
  function get_sub (type) { // @TODO: maybe remove?
    console.warn(`%c[STATE]%c[GET_SUB]`, `color: green;`, 'color: red;')
  ...
  //  }
  //  console.log('[watch]')
  //  const local = local_status
  //  return xtype === 'module' ? local.sub_modules : local.sub_instances[node_id]
  ...
  function get (sid) {
    ...
    if (!id) throw new Error(FAIL.nodrive_for_symbol({ sid }))
    return Object.assign(make_drive(id), { on, put: undefined })
...
/******************************************************************************
  [OK] HELPER: MAKE_DRIVE
******************************************************************************/
function make_drive (node_id) {
  // dependencies: db, FAIL, get_input
  const drive = {
    get: (path) => get(path, node_id),
    has: (path) => has(path, node_id),
    put: (path, buffer) => put(path, buffer, node_id),
    list: (path) => list(path, node_id),
  }
  return drive
  // --------------------------------------------------------------------------
  async function get (path, id = node_id) {
    ...
    const node = db.read(['state', id])
    let dataset
    if (!node.drive) throw new Error(FAIL.hasnodrive_fb ({ id: node.id }))
    ...
    if (!dataset) throw new Error(FAIL.dataset_not_found({
    ...
    if (!target_file) throw new Error(FAIL.nofile({ path, id: node.id }))
    return target_file
  }
  // --------------------------------------------------------------------------
  async function put (path, buffer, id = node_id) {
    ...
    const file = { id: raw_id, name: filename, type, raw: buffer }
    for (const file_id of dataset.files) {
  ...
  function list (path, id = node_id) {
    const node = db.read(['state', id])
    if (!node.drive) throw new Error(FAIL.hasnodrive({ id }))
    ...
    if (path) {
      ...
      if (index === undefined) throw new Error(FAIL.dataset_not_found({
        path, name: node.name
      }))
...
/******************************************************************************
  [OK] HELPER: MAKE_REGISTRY
******************************************************************************/
function make_registry () {
  // dependencies: db, status, console
  const importer = make_importer()
  const exporter = make_exporter()
  return { register, unregister, ...importer, ...exporter }
  // --------------------------------------------------------------------------
  function register ({ type: dataset_type, name: dataset_name, dataset }) {
    Object.entries(dataset).forEach(([node_id, files]) => {
      ...
      new_dataset.xtype = dataset_type // @TODO: Make dataset xtypes dynamic
    ...
    console.log('[STATE] registered ' + dataset_name + '.' + dataset_type)
  }
  // --------------------------------------------------------------------------
...
/******************************************************************************
  [OK] HELPER: MAKE_IMPORTER
******************************************************************************/
function make_importer () {
  return { import_db, import_root }
  // --------------------------------------------------------------------------
  ...
  function import_root (data, name) {
    Object.entries(data).forEach(([key, value]) => db.add([name, key], value))
    db.push(['root_datasets'], { name })
    status.callback({ type: 'import', data })
  }
}
/******************************************************************************
  [OK] HELPER: MAKE_EXPORTER
******************************************************************************/
function make_exporter () {
  return { export_db, export_root, get_dataset }
  // --------------------------------------------------------------------------
  function export_db (result) {
    ...
    _download(result, `${result.name}-${result.type}`)
  }
  // --------------------------------------------------------------------------
  function export_root ({ name }) {
    const result = db.read_all([name])
    console.log(`%c[STATE]`, `color: green;`, name, result)
    _download(result, name)
  }
  // --------------------------------------------------------------------------
  function _download (data, name) {
  ...
  // --------------------------------------------------------------------------
  function get_dataset ({ root = 'state', type: dataset_type, name: dataset_name } = {}) {
    ...
    if (dataset_type) {
      ...
      if (dataset_name) return recurse(status.ROOT_ID, dataset_type)
      return dataset_list
    }
    const datasets = []
    ...
    function recurse (node_id, dataset_type) {
      ...
      if (entry.drive) entry.drive.forEach(dataset_id => {
        const dataset = db.read(['state', dataset_id])
      ...
      if (!entry.subs) return node_list
      entry.subs.forEach(sub_id => node_list.push(...recurse(sub_id, dataset_type)))
      return node_list
    }
  }
  // --------------------------------------------------------------------------
...
function ERROR (url) {
  ...
  return {
    ...
    nomapping ({ dataset_type, subid, id, type }) {
...
/******************************************************************************
  HELPER: VALIDATE_FALLBACK_DATA
******************************************************************************/
// External Function (helper)
function validate_fallback_data (data) { // validates sanitized data
  ...
  const expected_structure = {
    'api::function': () => {},
    '_::object': {
      ":*:object|number": { ":*:function|string|object": '', "mapping::": {} }
    },
    'drive::object': {
      "::object": {
...
function validate_shape (obj, expected, super_node = 'root', path = '') {
  ...
  Object.entries(expected).forEach(([expected_key, expected_value]) => {
    ...
    if (expected_key_names) expected_key_names.split('|').forEach(expected_key_name => {
      ...
      if (value !== undefined) {
        ...
        if (expected_types.includes(type)) {
          if (type === 'object') {
            const new_path = path + '/' + expected_key_name
            validate_shape(value, expected_value, expected_key_name, new_path)
          }
        }
        else throw new Error(FAIL.type_mismatch({
      ...
      values.forEach((value, index) => {
        ...
        if (expected_types.includes(type)) {
          if (type === 'object') {
            const sub_path = path + '/' + keys[index]
            validate_shape(value, expected_value, keys[index], sub_path)
          }
        } else throw new Error(FAIL.type_mismatch({
    ...
    if (absent && required) {
      if (expected_key_names) throw new Error(FAIL.nokey({ expected_key_names, path }))
      else throw new Error(FAIL.nosupersubs({ super_node, path }))
    }
  })
  strict && keys.forEach(key => {
    if (!all_keys.includes(key)) throw new Error(FAIL.unknown_key({ key, path }))
  })
}
/*****************************************************************************/
