...
function STATE (address, modulepath, dependencies) {
  ...
  function validate_and_preprocess ({ fallback, xtype, pre_data = {}, orphan_check, fun_status, entries }) {
    ...
    let fallback_data = fallback(status.args[pre_id], { listify: tree => listify(tree, modulepath), tree: status.tree_pointers[modulepath] })
    try {
    ...
    if(fun_status.overrides[pre_id]){
      ...
      status.override_addresses[modulepath].push(status.addresses[fun_status.overrides[pre_id].by.at(-1).split(':')[0]])
