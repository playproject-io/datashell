...
const url = 'https://github.com/playproject-io/datashell/tree/main/doc'
const FALLBACK_POST_ERROR = `\nFor more info visit ${url}/state/temp.md#defining-fallbacks`
const FALLBACK_SYNTAX_POST_ERROR = `\nFor more info visit ${url}/state/temp.md#key-descriptions`
const FALLBACK_SUBS_POST_ERROR = `\nFor more info visit ${url}/state/temp.md#shadow-dom-integration`
...
function tolist (tree, prefix = '') {
  ...
  if (tree._ && typeof tree._ === 'object') walk(tree._, prefix)
  return result
}
/******************************************************************************
  ADMIN
...
function STATE (address, modulepath, dependencies) {
  ...
  **/
  function statedb (fallback) {
    if (!status.ROOT_ID) throw new Error('Admin access required! Call statedb.admin() first.' + FALLBACK_POST_ERROR)
...
//    if (!Object.values(status.open_branches).reduce((acc, curr) => acc + curr, 0)) status.inits.forEach(init => init())
    ...
    } catch (error) { throw new Error(`ID: ${modulepath}\n`+error) }
  }
...
//        status.open_branches[super_id]--
        ...
        while (!status.tree_pointers[new_super_id]) {
          ...
          if (temp_name === undefined) throw new Error(`STATE module must be initialized in  not properly initialized in preceeding path segments of the current "${id}" first, but so far has only been initialized in:\n* ${Object.keys(status.tree_pointers).join('\n* ')}`)
...
function create_statedb_interface (local_status, node_id, xtype) {
  ...
  function get_drive (sid){
    ...
    if (!id) throw new Error(`No drive found for symbol "${sid}"`)
    return {
  ...
  function list (path, id = node_id) {
    const node = db.read(['state', id])
    if (!node.drive) throw new Error(`Node "${id}" has no drive`)
